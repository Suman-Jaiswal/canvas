...
var colors = ['#F2935C', '#150773', '#A60F46', '#F27289', 'white']; // Event Listeners
...
function init() {
  for (var i = 0; i < controls.particlesCount; i++) {
    var speedFactor = Math.random() * controls.speedRange;
    var color = "hsl(".concat(i * 360 / controls.particlesCount, ", 100%, 80%)");
    var angle = Math.random() * Math.PI * controls.angle;
    var radius = Math.random() * 4 + 1;
    var velocity = {
      x: Math.cos(angle) * speedFactor,
      y: -Math.sin(angle) * speedFactor
    };
    objects.push(new _Object(mouse.x, mouse.y, radius, color, velocity));
  }
} // Animation Loop
